...
兩類技能：**工作流技能**（用 `/<name>` 觸發）與**能力技能**（自然語言觸發）。
...
| `/agents` | 管理 AI 角色定義（建立、切換、列出） |
| `/to-md` | 把 PDF／PPTX／DOCX／XLSX 轉成 Obsidian markdown 筆記並自動歸檔 |

### 能力技能（自然語言觸發）

來自 [Anthropic Skills API](https://github.com/anthropics/skills)，Claude 在符合場景時自動載入。

| 類別 | 技能 |
|------|------|
| 文件處理 | `pdf`、`docx`、`pptx`、`xlsx` |
| 視覺設計 | `canvas-design`、`frontend-design`、`brand-guidelines`、`theme-factory`、`algorithmic-art`、`slack-gif-creator` |
| 寫作溝通 | `doc-coauthoring`、`internal-comms` |
| 開發工具 | `claude-api`、`mcp-builder`、`skill-creator`、`web-artifacts-builder`、`webapp-testing` |

> 完整技能索引（含規劃中的工作流技能）：[System/skills.md](System/skills.md)
