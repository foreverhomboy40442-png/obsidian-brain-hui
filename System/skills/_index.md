...
- [`docx`](docx/SKILL.md) — Use this skill whenever the user wants to create, read, edit, or manipulate Word documents (`.docx`)
- [`xlsx`](xlsx/SKILL.md) — Use this skill any time a spreadsheet file is the primary input or output

### 視覺設計

- [`canvas-design`](canvas-design/SKILL.md) — Create beautiful visual art in `.png` and `.pdf` documents using design philosophy
- [`frontend-design`](frontend-design/SKILL.md) — Create distinctive, production-grade frontend interfaces with high design quality
- [`brand-guidelines`](brand-guidelines/SKILL.md) — Applies Anthropic's official brand colors and typography to any sort of artifact
- [`theme-factory`](theme-factory/SKILL.md) — Toolkit for styling artifacts with a theme
- [`algorithmic-art`](algorithmic-art/SKILL.md) — Creating algorithmic art using p5.js with seeded randomness and interactive parameter exploration
- [`slack-gif-creator`](slack-gif-creator/SKILL.md) — Knowledge and utilities for creating animated GIFs optimized for Slack

### 寫作與溝通

- [`doc-coauthoring`](doc-coauthoring/SKILL.md) — Guide users through a structured workflow for co-authoring documentation
- [`internal-comms`](internal-comms/SKILL.md) — A set of resources to help write all kinds of internal communications
- `naming` — Name products, SaaS, brands, open source projects, bots, and apps（未安裝）

### 開發工具

- [`claude-api`](claude-api/SKILL.md) — Build, debug, and optimize Claude API / Anthropic SDK apps
- [`mcp-builder`](mcp-builder/SKILL.md) — Guide for creating high-quality MCP (Model Context Protocol) servers
- [`skill-creator`](skill-creator/SKILL.md) — Create new skills, modify and improve existing skills, and measure skill performance
- [`web-artifacts-builder`](web-artifacts-builder/SKILL.md) — Suite of tools for creating elaborate, multi-component claude.ai HTML artifacts
...
|------|---|
| `/journal` `/agents` `/intake` `/maintain` `/digest` `/writing` `/reflect` `/to-md` | ✅ 已安裝 |
| `/cso` `/investigate` `/netbox` `/garage` `/scrape` `/k8s` `/mem0` `/slides` `/wagodanz` `/pm` | ❌ 尚未安裝（規劃中） |
